...
shinyServer(function(input, output) {
  ...
  output$table <- renderTable({
    ...
    movie_recommendation(input$select, input$select2, input$select3, input$year[1],input$year[2], c(input$genre1,input$genre2,input$genre3,input$genre4))
  })
  output$Movie_chosen <- renderUI({
                                  fluidRow(h4(input$Movie_chosen))
                                 
  })
 output$movie_desc<-renderUI({
   fluidRow(Movie_find[Movie_find$title==input$Movie_chosen,'overview'])
   })
 
 output$Image<-renderUI({
   poster_path<-Movie_find[Movie_find$title==input$Movie_chosen,'poster_path']
   div(img(src = poster_path , width = "300px", height = "300px",
           style="margin-top: -14px; margin-right:-14px;margin-left:-14px"))
 })
 output$Imdb_Link<-renderUI({
   
  a("IMDB Page",href= paste0('https://www.imdb.com/title/',Movie_find[Movie_find$title==input$Movie_chosen,'imdbId'] ))
